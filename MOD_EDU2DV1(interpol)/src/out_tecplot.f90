...
!   WRITE TECPLOT FILE
    ...
    write(1,*) 'title = "grid"'
    write(1,'(a100)') 'variables = "x","y","rho","rhou","rhov","rhoe","mach","Cp","Total Enthalpy","Entropy"'
    write(1,*) 'zone n=',nnodes,' e =', nelms,' et=quadrilateral, f=fepoint'

    ierr=0;allocate( lvars(4,nnodes),stat=ierr )
...
!   1. mach number
    ...
    lc = node(i)%dv(3)
    lvars(1,i) = sqrt(lu*lu+lv*lv)/lc
    enddo
    
!   2. cp (nondim)
    do i=1,nnodes
    lpress = node(i)%dv(1)
    lvars(2,i) = 2.0d0*(pinf-lpress)/(rhoinf*qinf*qinf)
    enddo
...
!   3. Total Enthalpy, H
    ...
    lu = node(i)%cv(2)/node(i)%cv(1) 
    lv = node(i)%cv(3)/node(i)%cv(1)
    lvars(3,i) = ggm1*node(i)%dv(1)/node(i)%cv(1) + 0.5d0*( lu*lu + lv*lv )
    enddo
    
!   4. entropy ( -1 form ) Kuya said to use rho?
    do i=1,nnodes
    lvars(4,i) = (node(i)%dv(1)/pinf)/( (node(i)%cv(1)/rhoinf)**gamma ) -1.0d0
    enddo
    
!   Store all data now
    do i = 1, nnodes
    write(1,*) node(i)%x,node(i)%y,(node(i)%cv(k),k=1,4),(lvars(k,i),k=1,4)
    end do
