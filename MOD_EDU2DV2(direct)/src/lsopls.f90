...
!    collocation array
     ...
     ierr=0 ; allocate( w(ntot+1),stat=ierr )
     if (ierr /= 0) pause 'lspls alloc error'
...
!    Centre Node Information
     ...
     do j=1,ntot 
      ...
      dr = dsqrt( H(1,jj)**2 + H(2,jj)**2 )
      w(jj) = 1.0d0/(dr**dble(gradient_weight_p))
     enddo
     HT=transpose(H)  ! reset & define

!    WEIGHTING MATRIX ( Diagonal )

     Kmat(1,1) = w(1) ! first diagonal component

     jj =1
     do j=1,ntot
      jj = jj + 1
      Kmat(jj,jj)=w(jj)  ! weighted diagonal components of neighbours
     enddo
