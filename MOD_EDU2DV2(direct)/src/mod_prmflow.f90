    ...
    real(p2), allocatable :: gcidnxy(:,:) ! array to set gcid cycle unit normals
    
    real(p2), allocatable :: nx_wc(:) ! i=1,nnodes boundary unit vector storage nx (not tied to node() since it will be deallocated )
    real(p2), allocatable :: ny_wc(:) ! i=1,nnodes boundary unit vector storage nx (not tied to node() since it will be deallocated )
